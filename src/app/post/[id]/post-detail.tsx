...
export default function PostDetailClient() {
  ...
  const handleShare = async () => {
    try {
      ...
      // User cancelled — copy to clipboard instead
      try {
        await navigator.clipboard.writeText(window.location.href);
      } catch {}
    }
...
function CommentThread({
  ...
  return (
    <div>
      ...
      {/* Nested replies */}
      {replies.length > 0 && (
        <div className="ml-8 mt-2 space-y-3 border-l-2 border-border/50 pl-3">
          {replies.map(reply => (
            <CommentItem
              key={reply.id}
              comment={reply}
...
function CommentItem({
  ...
  return (
    <div className="flex gap-3 group">
      <div className={`${isReply ? 'w-6 h-6' : 'w-8 h-8'} rounded-full bg-surface flex-shrink-0 overflow-hidden`}>
        {comment.user?.avatar_url ? (
          <img src={comment.user.avatar_url} alt="" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <span className={`${isReply ? 'text-[9px]' : 'text-xs'} text-taupe`}>{comment.user?.display_name?.charAt(0)}</span>
          </div>
        )}
      </div>
      <div className="flex-1">
        <div className="flex items-center justify-between">
          <p className={`${isReply ? 'text-[11px]' : 'text-xs'} font-inter font-medium text-ink`}>{comment.user?.display_name}</p>
        ...
        <p className={`${isReply ? 'text-[11px]' : 'text-xs'} text-ink/70 font-noto mt-0.5 leading-relaxed`}>{comment.content}</p>
        <div className="flex items-center gap-3 mt-1">
          ...
          {userId && !isDemo && (
            <button
              onClick={() => onReply(comment.id, comment.user?.display_name || '')}
