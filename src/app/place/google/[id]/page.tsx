...
// ═══ Google Place Detail Page ═══
// Accessed via /place/google/[googlePlaceId]
// Fetches directly from Google Places API (no Supabase lookup)

export default function GooglePlaceDetailPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error('Failed to load Google place:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const address = google.formattedAddress;
  ...
  const name = google.displayName?.text || '';
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="px-5 pb-24">
        ...
        <div className="pt-5 pb-4 border-b border-border">
          <h1 className="font-playfair italic text-2xl tracking-editorial text-ink leading-tight">{name}</h1>
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {rating && (
              <div className="flex items-center gap-1">
                <StarRating rating={rating} />
                <span className="text-sm font-inter font-medium text-ink">{rating.toFixed(1)}</span>
                {ratingCount && <span className="text-xs text-taupe font-inter">({ratingCount})</span>}
              </div>
            )}
            {price && <span className="text-xs text-taupe font-inter">{price}</span>}
            {hours && (
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-noto ${
                isOpen ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
              }`}>{isOpen ? '營業中' : '已打烊'}</span>
            )}
          </div>
          {tags.length > 0 && (
            <div className="flex gap-1.5 mt-3">
              {tags.map(tag => (
                <span key={tag} className="px-2.5 py-1 bg-surface border border-border rounded-full text-[10px] font-noto text-ink">{tag}</span>
              ))}
            </div>
          )}
          {summary && <p className="mt-3 text-sm text-taupe font-noto leading-relaxed">{summary}</p>}
        </div>
        ...
        <div className="py-4 space-y-3 border-b border-border">
          ...
          {hours?.weekdayDescriptions && hours.weekdayDescriptions.length > 0 && (
            <div>
              <button onClick={() => setHoursOpen(!hoursOpen)} className="w-full flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center flex-shrink-0">
                  <svg className="w-3.5 h-3.5 text-taupe" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                ...
                <svg className={`w-4 h-4 text-taupe transition-transform ${hoursOpen ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M6 9l6 6 6-6" />
                </svg>
              </button>
              {hoursOpen && (
          ...
          {website && (
            <a href={website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-ink font-inter truncate max-w-[240px]">{website.replace(/^https?:\/\//, '').replace(/\/$/, '')}</p>
              </div>
            </a>
          )}
        </div>
        ...
        {galleryPhotos.length > 0 && (
          <div className="py-5 border-b border-border">
            <h2 className="font-playfair italic text-base tracking-editorial text-ink mb-3">PHOTOS</h2>
            <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-5 px-5">
              {galleryPhotos.map((photo, i) => (
                <div key={i} className="w-40 h-40 flex-shrink-0 rounded-xl overflow-hidden bg-surface">
                  <img src={getPlacePhotoUrl(photo.name, 400)} alt={`${name} photo ${i + 2}`} className="w-full h-full object-cover" loading="lazy" />
                </div>
              ))}
            </div>
          </div>
        ...
        {reviews.length > 0 && (
          <div className="py-5 border-b border-border">
            <h2 className="font-playfair italic text-base tracking-editorial text-ink mb-3">REVIEWS</h2>
            <div className="space-y-4">
              {reviews.map((review, i) => (
                <div key={i} className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    {review.authorAttribution?.photoUri && (
                      <img src={review.authorAttribution.photoUri} alt="" className="w-6 h-6 rounded-full object-cover" />
                    )}
                    <span className="text-xs font-inter font-medium text-ink">{review.authorAttribution?.displayName || '匿名'}</span>
                    <StarRating rating={review.rating} size="small" />
                    <span className="text-[10px] text-taupe font-inter">{review.relativePublishTimeDescription}</span>
                  </div>
                  {review.text?.text && (
                    <p className="text-xs text-taupe font-noto leading-relaxed line-clamp-3">{review.text.text}</p>
                  )}
                </div>
              ))}
            </div>
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    stars.push(
      <svg key={i} className={`${starSize} flex-shrink-0`} viewBox="0 0 20 20">
        <path d="M10 1l2.39 4.84 5.34.78-3.87 3.77.91 5.33L10 13.27l-4.77 2.5.91-5.33L2.27 6.67l5.34-.78L10 1z" fill="#DDD8D0" />
        {fill > 0 && (
          <>
            <defs>
              <clipPath id={`gsc-${uid.current}-${i}`}>
                <rect x="0" y="0" width={fill === 1 ? 20 : 10} height="20" />
              </clipPath>
            </defs>
            <path d="M10 1l2.39 4.84 5.34.78-3.87 3.77.91 5.33L10 13.27l-4.77 2.5.91-5.33L2.27 6.67l5.34-.78L10 1z" fill="#D4A053" clipPath={`url(#gsc-${uid.current}-${i})`} />
          </>
        )}
      </svg>
    );
